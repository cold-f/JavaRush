...
public class Solution {
    private static AtomicInteger StaticNumberOfFactory = new AtomicInteger(1);
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        AtomicInteger NumberOfFactory;

        public AmigoThreadFactory() { //в конструкторе инкрементим дополнительную НЕ статическую
           NumberOfFactory = new AtomicInteger(StaticNumberOfFactory.getAndIncrement());
        }

        AtomicInteger numberOfThread = new AtomicInteger(1);
        @Override
        public Thread newThread(Runnable r)
        {
            ...
            thread.setName(thread.getThreadGroup().getName()+"-pool-"+NumberOfFactory.get()+"-thread-"+numberOfThread.getAndIncrement());
