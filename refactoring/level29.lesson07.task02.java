...
//c тернарным оператором выкидывало ошибку, потому что получалось так: Integer.valueOf(((Integer)null).intValue())
//дело том, что сначала тип расширяется по максимуму, а потом сужается до нужного.
